...
export default function MainControlBar({
  ...
  return (
    <div className={styles.controlBar}>
      ...
      <Link
        href={`${basePath}/troop`}
        className={`${styles.btn} ${myLevel >= 1 && nationLevel >= 1 ? '' : styles.disabled}`}
      >
        부대 편성
      </Link>
      <Link href={`${basePath}/diplomacy`} className={`${styles.btn} ${showSecret ? '' : styles.disabled}`}>
        외 교 부
      </Link>
      <Link href={`${basePath}/info/officer`} className={`${styles.btn} ${myLevel >= 1 ? '' : styles.disabled}`}>
        인 사 부
      </Link>
      <Link href={`${basePath}/nation/stratfinan`} className={`${styles.btn} ${showSecret ? '' : styles.disabled}`}>
        내 무 부
      </Link>
      <Link href={`${basePath}/chief`} className={`${styles.btn} ${showSecret ? '' : styles.disabled}`}>
        사 령 부
      </Link>
      <Link href={`${basePath}/npc-control`} className={`${styles.btn} ${showSecret ? '' : styles.disabled}`}>
        NPC 정책
      </Link>
      ...
      <Link href={`${basePath}/battle-center`} target="_blank" className={`${styles.btn} ${showSecret ? '' : styles.disabled}`}>
        감 찰 부
      </Link>
