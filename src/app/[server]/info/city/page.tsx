...
function CityInfoContent() {
  ...
  // URL에서 정렬 타입 읽기 (기본값: 10 = 시세)
  const type = Number(searchParams?.get('type') || '10');
  ...
  const updateParams = useCallback(
    (updates: Record<string, string | undefined>) => {
      ...
      Object.entries(updates).forEach(([key, value]) => {
        if (value === undefined || value === '' || (key === 'type' && value === '10')) {
          params.delete(key);
        } else {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 p-4 md:p-6 lg:p-8 font-sans">
      <div className="max-w-6xl mx-auto space-y-6">
        <TopBackBar title="세력 도시" reloadable onReload={loadCityList} />
        ...
        <FilterPanel>
          ...
          <div className="text-sm text-gray-400">
            총 <span className="text-white font-bold">{cityList.length}</span>개 도시
          </div>
        </FilterPanel>
          ...
          <div className="bg-gray-900/50 backdrop-blur-sm border border-white/5 rounded-xl overflow-hidden shadow-lg">
            ...
            <div className="md:hidden">
              <div className="divide-y divide-white/5">
                ...
                {cityList.length === 0 && (
                  <div className="py-8 text-center text-gray-500">도시가 없습니다.</div>
                )}
              </div>
            </div>
